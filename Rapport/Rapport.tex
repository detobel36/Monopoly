...
\section{Approche théorique}
  ...
  \subsection{Exemple de chaîne de Markov}
    ...
    \subsubsection{Représentation graphique}
      Il est également possible de représenter une chaîne de Markov comme
  ...
  \subsection{Probabilité}
    ...
    % Début version 3
%     La notion de probabilité est un sujet large et long à définit de manière complète 
%     et précise.  Il est cependant possible de définir ce qu'est une probabilité et
%     quelques conceptes basique lié à celle-ci.  Pour cela, on va s'intéresser à
%     l'occuerence d'un événemment.  En effet, dans un système, on peut définir un 
%     événement comme étant le résultat d'une action bien précise.  Par exemple,
%     tomber sur la face ``pile'' lorsqu'on lance une pièce de monaie est un événemment
%     aléatoire.  En opposition, la pièce pourrait tomber sur ``face'' et il s'agirait
%     alors d'un second événemment.
    
    
    % Début version 2
%     Dans une chaîne de Markov, on s'intéresse à la ``probabilité'' de se trouver dans un certain 
%     état ou la probabilité de passer d'un état à un autre.  De manière intuitive, une probabilité
%     peut être représenté comme étant la ``chance'' qu'un évenemment se produisse.
    % TODO expliquer les difficultés de définir une probabilité
    
    % Version 1
%     Définir formellement une probabilité
