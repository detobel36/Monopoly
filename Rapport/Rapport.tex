...
\begin{abstract}
  ...
  seront présentés et expliqués.  Les différents choix choix permettant 
  ...
  l'application et des résultats récupéré par l'implémentation de cette 
  modélisation sera également fait et permettra de déterminer les cases 
...
\section{Introduction}
  ...
  important de bien définir les chaînes de Markov ainsi que leurs propriété.
  Cette article se base donc essentiellement sur le cours: \citep{COURS}.
  Pour pouvoir être modélisé comme une chaîne de Markov, les règles du
  Monopoly doivent être clairement définit et certains choix doivent être
  fait pour permettre cette modélisation.  Une fois ces choix posé, cette
  article décrira les résultats obtenu grâce à l'implémentation à proprement
  parlé du Monopoly en chaîne de Markov.  Ces résultats permettrons de définir
  clairement quelles sont les cases les plus visitées mais également les
  cases les plus rentables.
...
\section{Approche théorique}
  ...
  \subsection{Les chaînes de Markov}
    ...
    de l'état d'un système aléatoire.  Les chaînes de Markov sont basés sur le 
    fait que l'état actuel du système dépend uniquement de l'état précédent.
    Cette propriété peut-être appellé ``propriété de Markov''.  Une chaîne de
    Markov est donc composé d'état et de lien entre ses différents états caractérisé
    par une certaine probabilité de passer d'un état A à un état B.
    Cette probabilité sera décrit plus en détail dans le point \ref{probabilite}.
    Il est possible de représenter une chaîne de Markov de manière matricielle ou
    même graphiquement.  Dans le cas matricielle, chaque colonne et chaque ligne
    correspond à un état.  Les éléments de cette matrice représente donc la probabilité
    de passer d'un état à un autre (respecivement l'état correspondant à la ligne vers
    l'état correspondant à la colonne).
    
    % TODO continuer en plus formel
    Une chaîne de Markov est une structure de donnée permettant de modéliser l'état 
    d'un système en se basant uniquement sur l'état précédent. La structure utilisée 
    dans ce rapport sera toujours considérée en temps discret.
    Une chaîne de Markov est composée de noeud et d'arrête reliant ces noeuds.  Chaque
  ...
  \subsection{Probabilité}
    \label{probabilite}
    Dans une chaîne de Markov, on s'intéresse à la ``probabilité'' de se trouver dans un certain 
  ...
  \subsection{Cas concret}
    ...
    \subsubsection{Représentation Graphique}
      Voici donc la chaîne de Markov décrivant cette situation:
      \begin{center}
	\begin{tikzpicture}
	  % Draw the states
    ...
    \subsubsection{Représentation matricielle}
      Une chaîne de Markov peut également être vue comme une matrice où chaque ligne et chaque 
	...
	\text{Hors service}\end{pmatrix}$$
      ...
      représentant la chaîne de Markov:
      $$
